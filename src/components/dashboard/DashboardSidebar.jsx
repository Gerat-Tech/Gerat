...
export default function DashboardSidebar({ stats = {}, isOpen = true, onClose = () => {} }) {
  ...
  return (
    <aside
      className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 bg-[#0d0d0d] border-r border-white/10 flex flex-col justify-between transition-transform duration-300 ${
    ...
    >
      ...
      <div className="flex flex-col">
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <Link href="/dashboard" className="flex items-center gap-3 group">
            <div className="size-8 rounded-[3px] bg-white/5 border border-white/15 flex items-center justify-center text-white group-hover:border-accent transition-colors">
            ...
            <div>
              <div className="font-roc text-sm font-bold tracking-tight uppercase text-white">
                GERAT CONTROL
              </div>
        ...
        <div className="px-5 py-2.5 bg-black/40 border-b border-white/5 flex items-center justify-between font-azeret text-[9px] tracking-[0.15em] text-white/40 uppercase">
              ...
              >
                <div className="flex items-center gap-3 truncate">
                  <svg
                    ...
                    fill="none"
                    stroke="currentColor"
                    className={`size-4 ${isActive ? "text-accent" : "text-white/40"}`}
                  >
                    <path
                      strokeLinecap="round"
                ...
                {badgeCount > 0 && (
                  <span className="px-1.5 py-0.5 rounded-[2px] bg-accent text-black font-azeret text-[9px] font-bold">
                    {badgeCount}
                  </span>
                )}
              </Link>
      ...
      <div className="p-4 border-t border-white/10 flex flex-col gap-2">
        <Link
          href="/"
          target="_blank"
          className="w-full py-2 px-3 bg-white/[0.02] hover:bg-white/[0.06] border border-white/10 text-white/70 hover:text-white font-azeret text-[9px] tracking-[0.15em] uppercase rounded-[2px] flex items-center justify-between transition-colors"
        >
